...
void create_axisIdxTable(WordManager *wordManager)
{
	...
	// if word num is not sufficient to select samples
	if(wordManager->wordNum <= SAMPLE_NUM) {
		...
		int wordCnt = 0;
		for(i=1; sampelCnt < SAMPLE_NUM && wordCnt; ++i) {
			int wordIdx;
			if(wordIdx = wordManager->wordIdxTable[i]) {
				if(wordManager->word[wordIdx].wordCnt < MIN_SAMPLE_WORD_CNT);
				else sampleWord[++sampleCnt] = wordManager->word[wordIdx];
				++wordCnt;
			}
		}
	}
	// sort sample words in ascending order (using reference index for data moving minimum)
	...
	if(sampleCnt * FILTER_RATE - 11 <= SIMULATION_NUM) {
		simulManager.word = sampleWord; freeFlag = false;
		simulManager.wordNum = simulManager.axisNum = sampleCnt;
		...
		simulManager.wordNum = simulManager.axisNum = SIMULATION_NUM;
		simulManager.wordIdxTable = (WordIdx*) calloc(MAX_HASH_SIZE, sizeof(WordIdx));
	...
	wordManager->axisNum = (simulManager.wordNum < AXIS_NUM ? simulManager.wordNum : AXIS_NUM);
	...
	if(freeFlag) free(simulManager->word);
	free(simulManager->wordIdxTable);
	free(simulManager->axisIdxTable);
	free(simulManager);
}
